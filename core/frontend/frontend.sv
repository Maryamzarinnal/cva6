...
#(
  ...
  // -----------------------------------------------------------------------
  // TC: classify the latched trace instructions
  // -----------------------------------------------------------------------
  always_comb begin
    int br_idx_scan;
    br_idx_scan = 0;
    ...
    for (int j = 0; j < TRACE_LEN; j++) begin
      ...
      t_cf = (!t_rvc && (t_instr[6:0] == riscv::OpcodeBranch ||
             ...
             ( t_rvc &&  t_instr[1:0]   == riscv::OpcodeC2 &&
                         ...
                         t_instr[6:2]   == 5'b00000 &&
                         t_instr[11:7]  != 5'b00000);
  ...
  // TC replay packing:
  // - pack from slot 0
  // - stop after first taken CF
  // - keep a map back to original trace indices
  // -----------------------------------------------------------------------
  always_comb begin
    logic found_start;
    logic stop_after_cf;
  ...
  // Original instr_queue uses scalar predict_address_i
  ...
  // -----------------------------------------------------------------------
  // Trace Cache record-side signals
  // -----------------------------------------------------------------------
